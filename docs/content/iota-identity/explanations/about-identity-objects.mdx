...
An on-chain identity is a digital representation of an entity’s identity, where with "entity" we refer to either a person,
...
On-chain Identities can have more than one controller too and the two control strategies mentioned above are instantiated
like so:
- [**indipendent control**](https://www.w3.org/TR/did-core/#independent-control): each controller accesses the identity
