...
use regulated_coin::{
    regulated_coin::{Self, AdminCap, REGULATED_COIN, SupplyManagerCap, Treasury}
};
use iota::{
...
fun test_supply_manager_mint_and_burn() {
    ...
    scenario.next_tx(admin_address);
    {
        ...
        let supply_manager_cap = regulated_coin::new_supply_manager(
            &mut treasury,
            &admin_cap,
            scenario.ctx()
        );
...
fun test_authorize_supply_manager_after_unauthorize() {
    ...
    scenario.next_tx(admin_address);
    {
        let mut treasury = scenario.take_shared<Treasury>();
        let admin_cap = scenario.take_from_sender<AdminCap>();
        regulated_coin::unauthorize_supply_manager(&mut treasury, &admin_cap);
        test_scenario::return_shared(treasury);
        scenario.return_to_sender(admin_cap);
    };
    ...
    scenario.next_tx(admin_address);
    {
        ...
        let sm_cap = scenario.take_from_address<SupplyManagerCap>(supply_manager_address);
        let sm_id = object::id(&sm_cap);
        regulated_coin::authorize_supply_manager(&mut treasury, &admin_cap, sm_id);
        // Return objects
        transfer::public_transfer(sm_cap, supply_manager_address);
        test_scenario::return_shared(treasury);
        scenario.return_to_sender(admin_cap);
    ...
    {
        let mut treasury = scenario.take_shared<Treasury>();
        let sm_cap = scenario.take_from_sender<SupplyManagerCap>();
        let deny_list = scenario.take_shared<DenyList>();
        regulated_coin::mint(&mut treasury, &sm_cap, &deny_list, 10, recipient_address, scenario.ctx());
        test_scenario::return_shared(treasury);
...
#[test, expected_failure(abort_code = regulated_coin::ESupplyManagerEntryAlreadyExists)]
...
fun test_reauthorize_old_after_new_fails() {
    ...
    scenario.next_tx(admin_address);
    {
        let mut treasury = scenario.take_shared<Treasury>();
        let admin_cap = scenario.take_from_sender<AdminCap>();
        let sm_cap2 = regulated_coin::new_supply_manager(&mut treasury, &admin_cap, scenario.ctx());
    ...
    scenario.next_tx(admin_address);
    {
        let mut treasury = scenario.take_shared<Treasury>();
        let admin_cap = scenario.take_from_sender<AdminCap>();
        regulated_coin::authorize_supply_manager(&mut treasury, &admin_cap, old_sm_id);
        test_scenario::return_shared(treasury);
        scenario.return_to_sender(admin_cap);
    };
...
fun test_events_emitted() {
    use regulated_coin::regulated_coin::{MintEvent, BurnEvent, PauseEvent, DenyListChangeEvent};
    let admin_address = @0xA;
    let supply_manager_address = @0xB;
    let recipient_address = @0xC;
    ...
    scenario.next_tx(supply_manager_address);
    {
        ...
        let sm_cap = scenario.take_from_sender<SupplyManagerCap>();
        let deny_list = scenario.take_shared<DenyList>();
        regulated_coin::mint(&mut treasury, &sm_cap, &deny_list, 100, recipient_address, scenario.ctx());
    ...
    scenario.next_tx(admin_address);
    {
        ...
        regulated_coin::pause_transfers(&mut treasury, &admin_cap, &mut deny_list, scenario.ctx());
        let pauses = event::events_by_type<PauseEvent>();
        assert!(vector::length(&pauses) == 1, 1100);
        assert!(regulated_coin::pause_event_enabled(&pauses[0]), 1101);
        test_scenario::return_shared(treasury);
        scenario.return_to_sender(admin_cap);
        test_scenario::return_shared(deny_list);
    };
    ...
    scenario.next_tx(admin_address);
    {
        ...
        let admin_cap = scenario.take_from_sender<AdminCap>();
        let mut deny_list = scenario.take_shared<DenyList>();
        regulated_coin::unpause_transfers(&mut treasury, &admin_cap, &mut deny_list, scenario.ctx());
        let pauses = event::events_by_type<PauseEvent>();
        assert!(vector::length(&pauses) == 1, 1200);
        assert!(!regulated_coin::pause_event_enabled(&pauses[0]), 1201);
        test_scenario::return_shared(treasury);
        scenario.return_to_sender(admin_cap);
        test_scenario::return_shared(deny_list);
    };
    ...
    scenario.next_tx(admin_address);
    {
        ...
        let admin_cap = scenario.take_from_sender<AdminCap>();
        let mut deny_list = scenario.take_shared<DenyList>();
        regulated_coin::block_address(&mut treasury, &admin_cap, &mut deny_list, deny_address, scenario.ctx());
        let changes = event::events_by_type<DenyListChangeEvent>();
        assert!(vector::length(&changes) == 1, 1300);
        assert!(regulated_coin::deny_list_change_event_address(&changes[0]) == deny_address, 1301);
        assert!(regulated_coin::deny_list_change_event_added(&changes[0]), 1302);
        test_scenario::return_shared(treasury);
        scenario.return_to_sender(admin_cap);
        test_scenario::return_shared(deny_list);
    };
    ...
    scenario.next_tx(admin_address);
    {
        ...
        let admin_cap = scenario.take_from_sender<AdminCap>();
        let mut deny_list = scenario.take_shared<DenyList>();
        regulated_coin::unblock_address(&mut treasury, &admin_cap, &mut deny_list, deny_address, scenario.ctx());
        let changes = event::events_by_type<DenyListChangeEvent>();
        assert!(vector::length(&changes) == 1, 1400);
        assert!(regulated_coin::deny_list_change_event_address(&changes[0]) == deny_address, 1401);
        assert!(!regulated_coin::deny_list_change_event_added(&changes[0]), 1402);
        test_scenario::return_shared(treasury);
        scenario.return_to_sender(admin_cap);
        test_scenario::return_shared(deny_list);
    };
    ...
    scenario.next_tx(supply_manager_address);
    {
        ...
        regulated_coin::burn(&mut treasury, &sm_cap, &deny_list, coin, scenario.ctx());
        let burns = event::events_by_type<BurnEvent>();
        assert!(vector::length(&burns) == 1, 1500);
        assert!(regulated_coin::burn_event_amount(&burns[0]) == amount, 1501);
        assert!(regulated_coin::burn_event_actor(&burns[0]) == supply_manager_address, 1502);
        test_scenario::return_shared(treasury);
        scenario.return_to_sender(sm_cap);
        test_scenario::return_shared(deny_list);
    };
...
fun test_total_supply() {
    ...
    scenario.next_tx(admin_address);
    {
        ...
        let cap_ref = regulated_coin::borrow_treasury_cap(&treasury);
    ...
    scenario.next_tx(supply_manager_address);
    {
        ...
        let sm_cap = scenario.take_from_sender<SupplyManagerCap>();
        let deny_list = scenario.take_shared<DenyList>();
        regulated_coin::mint(&mut treasury, &sm_cap, &deny_list, 200, recipient_address, scenario.ctx());
        test_scenario::return_shared(treasury);
        scenario.return_to_sender(sm_cap);
        test_scenario::return_shared(deny_list);
    };
    ...
    scenario.next_tx(supply_manager_address);
    {
        ...
        regulated_coin::burn(&mut treasury, &sm_cap, &deny_list, burn_part, scenario.ctx());
        test_scenario::return_shared(treasury);
        scenario.return_to_sender(sm_cap);
        test_scenario::return_shared(deny_list);
    };
...
#[test, expected_failure(abort_code = regulated_coin::EZeroAmount)]
...
fun test_block_unblock_address() {
    ...
    scenario.next_tx(admin_address);
    {
        ...
        let mut deny_list = scenario.take_shared<DenyList>();
        // Block address
        regulated_coin::block_address(&mut treasury, &admin_cap, &mut deny_list, blocked_address, scenario.ctx());
        test_scenario::return_shared(treasury);
        scenario.return_to_sender(admin_cap);
        test_scenario::return_shared(deny_list);
    };
    ...
    scenario.next_tx(admin_address);
    {
        ...
        let mut deny_list = scenario.take_shared<DenyList>();
        // Unblock address
        regulated_coin::unblock_address(&mut treasury, &admin_cap, &mut deny_list, blocked_address, scenario.ctx());
        test_scenario::return_shared(treasury);
        scenario.return_to_sender(admin_cap);
        test_scenario::return_shared(deny_list);
    };
...
#[test, expected_failure(abort_code = regulated_coin::EDeniedAddress)]
fun test_blocked_address_cannot_mint() {
    ...
    scenario.next_tx(supply_manager_address);
    {
        ...
        regulated_coin::mint(&mut treasury, &supply_manager_cap, &deny_list, 10, blocked_address, scenario.ctx());
        test_scenario::return_shared(treasury);
        scenario.return_to_sender(supply_manager_cap);
        test_scenario::return_shared(deny_list);
    };
...
#[test, expected_failure(abort_code = regulated_coin::EPaused)]
fun test_paused_transfers_fail() {
    ...
    scenario.next_tx(supply_manager_address);
    {
        ...
        regulated_coin::mint(&mut treasury, &supply_manager_cap, &deny_list, 10, recipient_address, scenario.ctx());
...
#[test, expected_failure(abort_code = regulated_coin::ENoSupplyManagerSet)]
...
fun test_update_and_verify_description() {
    ...
    scenario.next_tx(admin_address);
    {
        ...
        let metadata = regulated_coin::borrow_metadata(&treasury);
    ...
    scenario.next_tx(admin_address);
    {
        ...
        let mut metadata = regulated_coin::take_metadata(&mut treasury, &admin_cap);
        let treasury_cap = regulated_coin::borrow_treasury_cap_mut(&mut treasury, &admin_cap);
        update_description(treasury_cap, &mut metadata, new_description);
        regulated_coin::set_metadata(&mut treasury, &admin_cap, metadata);
        test_scenario::return_shared(treasury);
        scenario.return_to_sender(admin_cap);
    };
...
fun test_burn_zero_amount() {
    ...
    scenario.next_tx(supply_manager_address);
    {
        ...
        regulated_coin::mint(&mut treasury, &supply_manager_cap, &deny_list, 10, supply_manager_address, scenario.ctx());
        test_scenario::return_shared(treasury);
        scenario.return_to_sender(supply_manager_cap);
        test_scenario::return_shared(deny_list);
    };
    ...
    scenario.next_tx(supply_manager_address);
    {
        ...
        let supply_manager_cap = scenario.take_from_sender<SupplyManagerCap>();
        let deny_list = scenario.take_shared<DenyList>();
        regulated_coin::burn(&mut treasury, &supply_manager_cap, &deny_list, zero_coin, scenario.ctx());
        test_scenario::return_shared(treasury);
        scenario.return_to_sender(supply_manager_cap);
        test_scenario::return_shared(deny_list);
    };
...
fun test_create_second_supply_manager_fails() {
    ...
    scenario.next_tx(admin_address);
    {
        ...
        let supply_manager_cap_2 = regulated_coin::new_supply_manager(&mut treasury, &admin_cap, scenario.ctx());
...
fun test_unauthorize_and_create_new_supply_manager_works() {
    ...
    scenario.next_tx(admin_address);
    {
        ...
        let new_supply_manager_cap = regulated_coin::new_supply_manager(&mut treasury, &admin_cap, scenario.ctx());
    ...
    {
        let mut treasury = scenario.take_shared<Treasury>();
        let new_supply_manager_cap = scenario.take_from_sender<SupplyManagerCap>();
        let deny_list = scenario.take_shared<DenyList>();
        
        // New SupplyManager should be able to mint
        regulated_coin::mint(&mut treasury, &new_supply_manager_cap, &deny_list, 100, recipient_address, scenario.ctx());
...
#[test, expected_failure(abort_code = regulated_coin::ESupplyManagerNotAuthorized)]
fun test_old_supply_manager_fails_after_unauthorization() {
    ...
    scenario.next_tx(admin_address);
    {
        ...
        // Create first SupplyManager
        let old_supply_manager_cap = regulated_coin::new_supply_manager(&mut treasury, &admin_cap, scenario.ctx());
    ...
    {
        let mut treasury = scenario.take_shared<Treasury>();
        let old_supply_manager_cap = scenario.take_from_sender<SupplyManagerCap>();
        let deny_list = scenario.take_shared<DenyList>();
        
        // Old SupplyManager should fail to mint
        regulated_coin::mint(&mut treasury, &old_supply_manager_cap, &deny_list, 100, recipient_address, scenario.ctx());
...
fun test_burn_with_unauthorized_supply_manager_fails() {
    ...
    scenario.next_tx(supply_manager_address);
    {
        ...
        // Mint some coins
        regulated_coin::mint(&mut treasury, &supply_manager_cap, &deny_list, 100, supply_manager_address, scenario.ctx());
    ...
    {
        let mut treasury = scenario.take_shared<Treasury>();
        let unauthorized_cap = scenario.take_from_sender<SupplyManagerCap>();
        let coin = scenario.take_from_sender<Coin<REGULATED_COIN>>();
        let deny_list = scenario.take_shared<DenyList>();
        
        // Should fail because SupplyManager is no longer authorized
        regulated_coin::burn(&mut treasury, &unauthorized_cap, &deny_list, coin, scenario.ctx());
...
fun test_old_supply_manager_burn_fails_with_new_active() {
    ...
    scenario.next_tx(old_supply_manager_address);
    {
        ...
        regulated_coin::burn(&mut treasury, &old_supply_manager_cap, &deny_list, coin, scenario.ctx());
